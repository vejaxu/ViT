...
def get_dataloader(batch_size):
    ...
    train_dataset = torchvision.datasets.CIFAR10('./dataset/cifar10/p10_dataset', train=True, transform=data_transform["train"], download=True)
    test_dataset = torchvision.datasets.CIFAR10('./dataset/cifar10/p10_dataset', train=False, transform=data_transform["val"], download=True)
...
def get_net(): # 获得预训练模型并冻住前面层的参数
    ...
    net = vit.ViT(image_size=224, patch_size=16, num_classes=10, dim=32, depth=2, heads=4, mlp_dim=32, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.)
    ...
    # net.transformer.weight.requires_grad = True
    # net.transformer.bias.requires_grad = True
    return net
...
def train(net, loss, train_dataloader, valid_dataloader, device, batch_size, num_epoch, lr, lr_min, optim='sgd', init=True, scheduler_type='Cosine'):
    ...
    if optim == 'sgd':
        ...
        optimizer = Ranger((param for param in net.parameters() if param.requires_grad), lr=lr, weight_decay=0)
    ...
    for epoch in range(num_epoch):
        ...
        with torch.no_grad():
            ...
            if eval_acc > best_acc:
                best_acc = eval_acc
                torch.save(net.state_dict(),'dataset/cifar10/cifar10.pth')
